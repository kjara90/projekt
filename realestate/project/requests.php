...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Kërkesa u fshi!';
   }else{
      $warning_msg[] = 'Kërkesa është fshirë!';
   }
...
<section class="requests">
   ...
   ?>
   <div class="box">
      <p>Emri : <span><?= $fetch_sender['name']; ?></span></p>
      <p>Numri i telefonit : <a href="tel:<?= $fetch_sender['number']; ?>"><?= $fetch_sender['number']; ?></a></p>
      <p>email : <a href="mailto:<?= $fetch_sender['email']; ?>"><?= $fetch_sender['email']; ?></a></p>
      <p>Kërkesa : <span><?= $fetch_property['property_name']; ?></span></p>
      <form action="" method="POST">
         ...
         <a href="view_property.php?get_id=<?= $fetch_property['id']; ?>" class="btn">Shiko pronën</a>
      </form>
   </div>
   <?php
    }
   }else{
      echo '<p class="empty">Nuk ke asnjë kërkesë!</p>';
   }
   ?>
