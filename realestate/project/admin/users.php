...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Fshi përdoruesin!';
   }else{
      $warning_msg[] = 'Përdoruesi u fshi!';
   }
...
<section class="grid">
   ...
   <h1 class="heading">Përdoruesit</h1>

   <form action="" method="POST" class="search-form">
      <input type="text" name="search_box" placeholder="Kërko përdorues..." maxlength="100" required>
   ...
   ?>
   <div class="box">
      <p>Emri : <span><?= $fetch_users['name']; ?></span></p>
      <p>Numri i telefonit : <a href="tel:<?= $fetch_users['number']; ?>"><?= $fetch_users['number']; ?></a></p>
      <p>Email : <a href="mailto:<?= $fetch_users['email']; ?>"><?= $fetch_users['email']; ?></a></p>
      <p>Pronat e listuara : <span><?= $total_properties; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="delete_id" value="<?= $fetch_users['id']; ?>">
         <input type="submit" value="Fshi përdoruesin" onclick="return confirm('delete this user?');" name="delete" class="delete-btn">
      </form>
   </div>
   <?php
      }
   }elseif(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
      echo '<p class="empty">Rezultati nuk u gjet!</p>';
   }else{
      echo '<p class="empty">Akoma nuk është shtuar asnjë user!</p>';
   }
   ?>
