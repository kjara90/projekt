...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      ...
      if($verify_name->rowCount() > 0){
         $warning_msg[] = 'Username është përdorur!';
      }else{
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            ...
            $warning_msg[] = 'Ju lutem vendosni password të ri!';
         }
      }
   }
...
<section class="form-container">
   ...
   <form action="" method="POST">
      ...
      <input type="password" name="old_pass" placeholder="Vendos password-in e vjetër" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
      <input type="password" name="new_pass" placeholder="Vendos password-in e ri" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
      <input type="password" name="c_pass" placeholder="Konfirmo password-in" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
